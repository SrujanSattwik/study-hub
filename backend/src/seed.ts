 ...
 * Run via: cd backend && npm run prisma:seed
 *
    ...
    if (!annExists) {
      await prisma.groupAnnouncement.create({
        data: {
          ...
          content:
            "This group is for students tackling Calculus I & II. Share resources, ask questions, and support each other. Weekly problem sessions every Monday 7pm.",
          createdBy: adminId,
        },
      });
  ...
  const notifications = [
    {
      ...
      content:
        "Your account is set up and ready. Explore study groups and connect with peers.",
    },
    {
      userId: carolId,
      type: "event",
